...
import com.tsumundev.gundurability.init.GundurabilityModGameRules;
import com.tsumundev.gundurability.init.GundurabilityModSounds;
import com.tsumundev.gundurability.util.GunDurabilityConstants;
...
public class TaczEvents {
    ...
    private static double getBiomeModifier(LivingEntity shooter) {
        try {
            ...
            if (isInWater) {
                return Config.RIVER_JAM_MULTIPLIER.get() * 1.5;
            }

            boolean isRaining = shooter.level().isRaining();
            if (isRaining) {
                return Config.RAIN_JAM_MULTIPLIER.get();
            }
